...
public class LeetCodeAlgorithm {
    ...
    public void twoSum() {
        ...
        int target = 9;
        Integer[] searchNums = new Integer[2];
        ...
        System.out.println("数组中不存在符合条件的项");
    }
     ...
     */
    @Test
    public void AddTwoNumb() {
        String a = "3,4,2";
        String b = "4,6,5";

        List<String> listA = Arrays.asList(String.valueOf(a).split(","));
        List<String> listB = Arrays.asList(String.valueOf(b).split(","));
        Collections.reverse(listA);
        Collections.reverse(listB);

        int indexNum = 0;//个位相加如果大于10， 存储起来给后面加
        ...
        for(int i=0; i<longerList.size(); i++) {
            ...
            int resultNum = 0;
            if(shortList.size()>i) {
                numShort = Integer.parseInt(shortList.get(i));
            }
            if(longerList.size()>i) {
                numLong = Integer.parseInt(longerList.get(i));
            }

            resultNum = indexNum + numLong + numShort;
            if(resultNum >= 10) {
                indexNum = Integer.parseInt(String.valueOf(resultNum).substring(0, 1));
                resultNum = Integer.parseInt(String.valueOf(resultNum).substring(1));
            } else {
                indexNum = 0;
            }
            resultList.add(String.valueOf(resultNum));
        }

        if(indexNum > 0) {
            resultList.add(String.valueOf(indexNum));
        }

        System.out.println(GsonUtils.toJson(resultList));
    }
     ...
     */
    @Test
    public void longestStringWithOutRepeat() {
        String inputStr = "abcabcbb";

        int targetLength = inputStr.length();
        String targetStr = "";
        while (targetLength>0) {
            for(int i=0; i<inputStr.length()-targetLength+1; i++) {
                targetStr = inputStr.substring(i, targetLength);
                //判断目标targetStr是否有重复
                for(int j=0; j<targetStr.length(); j++) {
                    if(targetStr.indexOf(targetStr.substring(j, j+1)) != targetStr.lastIndexOf(targetStr.substring(j, j+1))) {
                        //有重复，跳出循环
                        break;
                    }
                    if(j==targetLength - 1) {
                        System.out.println(targetLength + "--targetStr:" + targetStr);
                        return;
                    }
                }
            }
     ...
     */
    @Test
    public void getLongestPalindromicString() {
        String inputStr = "bacb1cad";

        String longestStr = "";

        for(int i=0; i<inputStr.length()-1; i++) {
            int firstNum = inputStr.indexOf(inputStr.charAt(i));
            int lastNum = inputStr.lastIndexOf(inputStr.charAt(i));
            if(firstNum!=lastNum) {
                String currentLongestStr = inputStr.substring(firstNum, lastNum) + inputStr.charAt(i);
                if(isHuiWen(currentLongestStr) && currentLongestStr.length() > longestStr.length()) {
    ...
    public boolean isHuiWen(String inputStr) {
...
//        return false;
     ...
     *
     * a00 a10 a20 a30-a21 a12   a03 a13 a23 a33-a24 a15   a06 a16
     */
    @Test
    public void snakeString() {
        String inputString = "ABC";//length-----14
        int numRows = 2;
        int integerNum = (inputString.length()/(numRows + numRows - 2)) * (numRows - 1);
        int dotNum = (inputString.length()%(numRows + numRows - 2)) > numRows ? (inputString.length()%(numRows + numRows - 2))-numRows + 1 : 1;
        int numColumns = integerNum + dotNum;
        System.out.println("numColumns:" + numColumns + "  integerNum:" + integerNum + " dotNum:" + dotNum);
        int charIndex = 0;
        ...
        for(int i=0; i<numColumns; i++) {
            ...
            for(int j=0; j<numRows; j++) {
                ...
                if (charIndex<inputString.length()) {
                    if(i%(numRows-1)==0) {
                        //此列全都有数字
                        a[j][i] = inputString.charAt(charIndex);
                        charIndex++;
        ...
        for(int i=0; i<a.length; i++) {
            for(int j=0; j<a[i].length; j++) {
                String stringInArray = String.valueOf(a[i][j]);
                if(stringInArray!=null && stringInArray.length()>0) {
                    returnStr.append(String.valueOf(a[i][j]).trim());
                }
            }
        }
     ...
     */
    @Test
    public void testConcatMiddle() {
        int[] num1 = new int[]{1, 2};
        int[] num2 = new int[]{3, 4};
        int[] newNumArray = new int[num1.length+num2.length];
        int num1Index = 0;
        int num2Index = 0;
        int newNumIndex = 0;
        while (newNumIndex<newNumArray.length) {
            if(num1Index<num1.length && num2Index<num2.length) {
                if (num1[num1Index]>num2[num2Index]) {
                    newNumArray[newNumIndex] = num2[num2Index];
                    num2Index++;
                } else {
                    newNumArray[newNumIndex] = num1[num1Index];
                    num1Index++;
                }
            } else if(num1Index < num1.length) {
                newNumArray[newNumIndex] = num1[num1Index];
                num1Index++;
        ...
        System.out.println(GsonUtils.toJson(newNumArray));
    }
    ...
    /**
     * 15. 三数之和
     * 耗时：11s698ms...--->9s623ms--->8s747ms ---> 6s969ms----->2s784ms--->219ms
     *
     *
     *
     * 官方  189ms
...
//        int[] nums = {1,-1,-1,0};
        System.out.println(GsonUtils.toJson(threeSum(nums)));
    }

    public List<List<Integer>> threeSum(int[] nums) {
        Arrays.sort(nums);
        List<List<Integer>> returnNumList = new ArrayList();
        for(int i=0; i<nums.length; i++) {
            if(nums[i]>0 || (i>0 && nums[i]==nums[i-1])) {
                continue;
            }
            int k = nums.length-1;//k一定会在上一个k（k'）的左侧
            for(int j=i+1; j<nums.length; j++) {
...
//        return ans;
    ...
    public List<Integer> getOrderList(int a ,int b, int c) {
//        int[] intArray = {a, b, c};
...
//        Arrays.asList(intArray);
    ...
    public boolean isRepeat(List<Integer> currentList, List<List<Integer>> returnNumList) {
        for(int i=0; i<returnNumList.size(); i++) {
            if(currentList.get(0) == returnNumList.get(i).get(0) && currentList.get(1) == returnNumList.get(i).get(1) && currentList.get(2) == returnNumList.get(i).get(2)) {
                return true;
            }
        }
        return false;
    ...
    public String longestPalindrome(String s) {
        ...
        char firstRepeatChar = s.charAt(0);
        String maxStr = String.valueOf(firstRepeatChar);
        int maxLength = 0;
        int lastIndex = s.length()-1;

        for(int firstIndex=0; firstIndex<s.length()-1 && lastIndex-firstIndex+1>maxLength; firstIndex++) {
            while(lastIndex>firstIndex) {
                if(isHuiWen(s.substring(firstIndex, lastIndex+1))) {
                    //回文
                    if(lastIndex - firstIndex + 1 > maxLength) {
                        //新的回文更长
                        maxStr = s.substring(firstIndex, lastIndex + 1);
